    ...
    <div className="grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-4">
      {HIGHLIGHTS.map(({ icon, title, desc }) => (
        ...
        >
          ...
          <span className="font-serif text-base sm:text-lg text-[var(--color-on-deep)] tracking-[0.25em]">
            {title}
          </span>
          <span className="text-xs sm:text-sm text-[var(--color-on-deep)] opacity-60 mt-1.5 leading-relaxed">
            {desc}
          </span>
        </div>
      ))}
