...
export default function MenuItemRow({ item, realModeEnabled = false }: Props) {
  ...
  return (
    ...
    >
      {/* 縮圖 */}
      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded bg-[var(--color-bg-card-hover)] flex-shrink-0 overflow-hidden">
        {item.imageUrl ? (
          <img
            src={item.imageUrl}
            alt={item.name}
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[var(--color-text-primary)] font-bold text-base sm:text-lg md:text-xl content-text tracking-wide">
      ...
      <div className="flex-shrink-0 text-right">
        <div className="text-[var(--color-gold-primary)] text-sm sm:text-base font-semibold content-text">{item.price} gil</div>
