...
## Stage 3 第一批完成狀態(2026-07-17)

5 個 Task 全數完成並個別通過 review(T4 有一輪 fix 後 re-review 通過)。使用者本機 `npm run dev` 互動走查通過(側欄廣告、跑馬燈、訪客計數器)。走查過程中額外發現並修復 5 個問題(均為本批次功能或既有功能的既存缺陷,非本批次規劃內但同會話一併處理):

1. 彈窗上傳圖片未走壓縮管線(宣傳照輪播上傳)
2. 後台留言遮蔽審核看不到原文、附註只能用 prompt 一次性設定
3. `GlobalSettings.photoUrls` 從未實際渲染(新增 Hero 輪播)
4. 宣傳照/彈窗刪除只清 Firestore、Storage 檔案從未真正刪除(orphan)
5. `marqueeText` 讀取端漏欄位,存了顯示不出來
6. 跑馬燈原本靜態嵌在頁面中、文字短時視覺上幾乎不動,改為 `fixed bottom-0` 貼齊視窗底部,並改用 `translateX(100vw)→-100%` 確保任何長度文字都完整橫越畫面,速度 20s→10s
7. `Navbar`/`Footer` 掛在路由外層不重掛載,後台改地址後前台要整頁重整才會更新,改用 `onSnapshot` 即時訂閱

測試 8/8、build 綠。全部 commit 完成,尚未 push,待使用者確認後自行部署。
